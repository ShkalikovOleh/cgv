...
template <typename T> class array3d_t
{
  ...
  public:
	...
	T* begin() { return arr_data; }

	T* end() { return arr_data + dim_xy * dim_z; }

	const T* begin() const { return arr_data; }

	const T* end() const { return arr_data + dim_xy * dim_z; }

	// setter getter
	inline const int size_x() const { return dim_x; }

	inline const int size_y() const { return dim_y; }

	inline const int size_z() const { return dim_z; }

	inline const T* data() const { return arr_data; }
};
...
class DT3D
{
  public:
	...
	template <typename T> float distance(T _x, T _y, T _z);
...
// template functions
template <typename T> inline float DT3D::distance(T _x, T _y, T _z)
{
	...
	return std::sqrt(a * a + b * b + c * c) / scale + A(x, y, z).distance;
}
